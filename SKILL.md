...
description: >-
  ...
  结构分析。技能会跑完整流程：按一手披露优先级探测数据并自动降级标注；以预期差（市场隐含 vs 独立预期）为脊柱，产出结构化九章研报——估值一律由脚本计算，反向
  DCF+PVGO、三情景概率加权、EPV、EVA/剩余收益多法交叉验证并可跑蒙特卡洛；财报质量做取证会计检查（应计/M-Score）；结论按预注册标定规则映射并经红队复核；财报请求自动进入同等深度财报模式——并保存为带来源与时间戳的文件。Do
...
- **先验真后估值**：财报可信度（取证会计）先于估值；可疑的利润不值得折现。
...
### Step 4 · 估值（多方法交叉验证）
- **至少三种方法**，顺序：反向 DCF+PVGO 开篇（现价隐含什么）→ 三情景概率加权 DCF（可加蒙特卡洛）→ 三要素/EPV → EVA/剩余收益 → 相对估值（应得倍数纪律）/SOTP/行业特定法。
- **所有计算一律 `scripts/dcf.py` 执行（假设写 JSON），禁止心算**；折现率构建按 `cost-of-capital.md`，全报告同源；终值执行"防作弊三查"；关键假设标 base rate 分位。
...
### Step 4.5 · 红队与洞见闸门（成稿前，必做）
- **洞见闸门**：与共识的最大分歧、市场为何犯错、最早何时能发现自己错了——三问答不出，动作降为"观望"。
- **红队/Pre-mortem**："一年后失败的 3 个最可能原因"，至少一条直击本报告核心论点；写入第九章。条件允许时用独立子 agent 攻击草稿论点后再定稿。
...
- `references/expectations-investing.md` — 预期差脊柱：反向 DCF/PVGO/Gap 表/洞见闸门。**估值与第一章必读。**
- `references/forensic-accounting.md` — 取证会计与财报可信度等级。**Step 2 必读。**
...
- `scripts/check_research_output.py` — 一致性+取证检查器。

## 四、质量自检（成稿前）

- 结论框/Tearsheet/Gap 表是否齐备且三处标签一致（第一章、第九章、标定规则可复算）？
- 预期差是否可证伪？洞见闸门三问是否有实答？红队是否至少一条直击核心论点？
...
- 收尾三件套：现金测试问题、监控清单（3–5 项带阈值）、置信度自评表。
